...
import type { JSX, SubmitEvent, ChangeEvent } from 'react';

type Field = 'fullname' | 'username' | 'email' | 'password';
...
export default function RegisterForm(): JSX.Element {
  ...
  const handleSubmit = async (e: SubmitEvent<HTMLFormElement>) => {
    ...
    const checkForm = Validation.register(form);
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col text-platinum/85">
      <div className="flex flex-col gap-1">
        ...
        />
        <ErrorValue text={errors['fullname']} />
      </div>
      <div className="flex flex-col gap-1">
        <label htmlFor="username" className="ml-2 font-semibold">
          Username
        </label>
        <input
          type="text"
          ...
          className="h-12 outline-none rounded-xl font-normal px-3 bg-raisin-black border-[0.05rem] border-spanish-gray"
        />
        <ErrorValue text={errors['username']} />
      </div>
      <div className="flex flex-col gap-1">
      ...
      <div className="relative flex flex-col gap-1">
        ...
        <input
          ...
          className="h-12 outline-none rounded-xl font-normal px-3 bg-raisin-black border-[0.05rem] border-spanish-gray"
        />
        <ErrorValue text={errors['password']} />
        <button
          type="button"
      ...
      <SubmitButton label="Create Account" mt={6} is_loading={loading} />
    </form>
  );
}
