...
export default function LoginForm(): JSX.Element {
  ...
  const handleSubmit = async (e: SubmitEvent<HTMLFormElement>) => {
    ...
    const checkForm = Validation.login(form);
    ...
    <form onSubmit={handleSubmit} className="flex flex-col text-platinum/85">
      <div className="flex flex-col gap-1">
        ...
        <input
          ...
          className="h-12 outline-none rounded-xl font-normal px-3 bg-raisin-black border-[0.05rem] border-spanish-gray placeholder:text-sm"
        />
        <ErrorValue text={errors['identifier']} />
      </div>
      <div className="relative flex flex-col gap-1">
        ...
        <button
          ...
          className="absolute right-3 bottom-9.5 hover:cursor-pointer"
        >
          {view ? (
            <svg
        ...
        <Link
          to="/forgot-password"
          className="absolute right-1 -bottom-2 font-semibold text-sm hover:underline"
        >
          Forgot password?
        </Link>
      </div>
      <SubmitButton label="Log In" mt={8} is_loading={loading} />
    </form>
  );
}
