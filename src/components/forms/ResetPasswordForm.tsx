...
export default function ResetPasswordForm(): JSX.Element {
  const [form, setForm] = useState<Record<Field, string>>(defaultValue);
  ...
  const handleSubmit = async (e: SubmitEvent<HTMLFormElement>) => {
    ...
    const checkForm = Validation.resetPassword(form);

    if (checkForm) {
      return setErrors((prev) => ({ ...prev, ...checkForm }));
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col font-semibold text-platinum/85">
      <div className="relative flex flex-col gap-1">
        <label htmlFor="new_password" className="ml-2">
          New Password
        </label>
        <input
          type={view.new_password ? 'text' : 'password'}
          id="new_password"
          name="new_password"
          ...
          className="h-12 outline-none rounded-xl font-normal px-3 bg-raisin-black border-[0.05rem] border-spanish-gray"
        />
        <ErrorValue text={errors['new_password']} />
        <button
          type="button"
          onClick={() => setView((prev) => ({ ...prev, new_password: !prev.new_password }))}
      ...
      <SubmitButton label="Update Password" mt={4} is_loading={loading} />
    </form>
  );
}
