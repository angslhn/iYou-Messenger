...
export default function DeleteAccount({ onBack }: Props): JSX.Element {
  ...
  return (
    <SectionLayout>
      <nav className="h-14 flex items-center gap-4 px-6 border-b border-ebony-light">
        ...
        <h1 className="font-semibold text-xl text-platinum/85">Delete Account</h1>
      </nav>
      <div className="px-5 w-full flex flex-col gap-6 py-6">
        <div className="flex flex-col gap-3 p-4 rounded-2xl bg-red-600/5 border border-red-600/20">
          <div className="flex items-center gap-3">
            <div className="size-10 shrink-0 flex justify-center items-center rounded-xl bg-red-600/10 border border-red-600/20">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 256 256"
                className="size-5 fill-red-500/80"
              >
                <path d="M236.8,188.09,149.35,36.22a24.76,24.76,0,0,0-42.7,0L19.2,188.09a23.51,23.51,0,0,0,0,23.72A24.35,24.35,0,0,0,40.55,224h174.9a24.35,24.35,0,0,0,21.33-12.19A23.51,23.51,0,0,0,236.8,188.09ZM120,104a8,8,0,0,1,16,0v40a8,8,0,0,1-16,0Zm8,88a12,12,0,1,1,12-12A12,12,0,0,1,128,192Z" />
              </svg>
            </div>
            <h2 className="font-bold text-red-500/90 leading-tight">This action is permanent</h2>
          </div>
          <ul className="flex flex-col gap-1.5 ml-1">
            {[
              'Your account and profile will be permanently removed',
              'All messages, groups, and stories will be deleted',
              'Your friends list and connections will be lost',
              'This cannot be undone or recovered',
            ].map((item) => (
              <li key={item} className="flex items-start gap-2 text-sm text-red-400/70">
                <span className="mt-1 size-1.5 rounded-full bg-red-500/50 shrink-0" />
                {item}
              </li>
            ))}
          </ul>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-2">
          <p className="ml-1 text-sm text-platinum/50 mb-2">
