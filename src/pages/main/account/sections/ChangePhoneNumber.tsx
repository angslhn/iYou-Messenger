...
export default function ChangePhoneNumber({ onBack }: Props): JSX.Element {
  ...
  const handleSubmitForm = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    const checkPhone = Validation.phone(newPhone);
    ...
    if (newPhone === user?.phone) {
      setPhoneError('New phone number cannot be the same as current phone number');
      return;
    }
    ...
    try {
      const {
        data: { token },
      } = await api.patch('/users/phone', { phone: newPhone });
    ...
    return (
      <SectionLayout>
        <nav className="h-14 flex items-center gap-4 px-6 border-b border-ebony-light">
          ...
          <h1 className="font-semibold text-xl text-platinum/85">Verify New Phone Number</h1>
        </nav>
        ...
        <div className="px-6 w-full flex flex-col items-center">
          <form onSubmit={handleVerify} className="w-full flex flex-col items-center gap-6 py-8">
            ...
            <SubmitButton label="Confirm New Phone Number" mt={0} is_loading={isLoading} />
          ...
          <div className="relative flex flex-col gap-1">
            ...
            {phoneError === '' && (
              <span className="absolute -bottom-2 px-2 text-sm text-platinum/50">
                Enter your new phone number. A verification code will be sent to your email.
              </span>
            )}
          </div>
