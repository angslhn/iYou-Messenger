...
const defaultForm = { current_password: '', new_password: '', confirm_password: '' };
const defaultErrors = { current_password: '', new_password: '', confirm_password: '' };

type Props = { onBack: () => void };

export default function ChangePassword({ onBack }: Props): JSX.Element {
  const [form, setForm] = useState<Record<Field, string>>(defaultForm);
  const [errors, setErrors] = useState<Record<Field, string>>(defaultErrors);
  const [view, setView] = useState<Record<Field, boolean>>({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setErrors(defaultErrors), 3500);
  ...
  const handleSubmit = async (e: React.SubmitEvent<HTMLFormElement>) => {
    ...
    const checkCurrent = !form.current_password.trim() ? 'Current password cannot be empty' : null;

    const checkNew = Validation.password(form.new_password);

    const checkConfirm =
      form.confirm_password !== form.new_password ? 'Passwords do not match' : null;

    if (checkCurrent || checkNew || checkConfirm) {
      setErrors({
        current_password: checkCurrent ?? '',
        new_password: checkNew ?? '',
        confirm_password: checkConfirm ?? '',
      });
      return;
    }
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        ...
        if (msg) setErrors((prev) => ({ ...prev, current_password: msg }));
      }
    } finally {
      setIsLoading(false);
    }
  ...
  const EyeOpen = () => (
    <svg className="w-5 fill-platinum/85" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      <path
        fillRule="evenodd"
        clipRule="evenodd"
  ...
  const EyeClosed = () => (
    <svg
      className="w-5 mt-1.5 fill-platinum/85"
      viewBox="0 0 24 24"
  ...
  return (
    <SectionLayout>
      <nav className="h-14 flex items-center gap-4 px-6 border-b border-ebony-light">
        ...
        <h1 className="font-semibold text-xl text-platinum/85">Update Password</h1>
      </nav>
        ...
        <form onSubmit={handleSubmit} className="w-full flex flex-col gap-2 py-6">
          {fields.map(({ field, label, hint }) => (
            <div key={field} className="relative flex flex-col gap-1">
              ...
              <button
                type="button"
                onClick={() => toggleView(field)}
                className="absolute right-3 bottom-9 hover:cursor-pointer"
              >
                {view[field] ? <EyeOpen /> : <EyeClosed />}
              </button>
            </div>
          ))}
          <SubmitButton label="Update Password" mt={0} is_loading={isLoading} />
        </form>
      </div>
    </SectionLayout>
  );
