...
export function password(value: string): string | null {
  ...
  if (isLength === 0) return 'Password cannot be empty';
...
export function email(value: string): string | null {
  ...
  if (!isValidFormat) return 'The email format used is invalid';
...
export function fullname(value: string): string | null {
  ...
  if (!trimmed) return 'Fullname cannot be empty';

  // Cek panjang minimum dan maksimum sesuai kolom DB
  if (trimmed.length < 1) return 'Fullname must be at least 1 character';
  if (trimmed.length > 60) return 'Fullname is too long, maximum 60 characters';

  // Cek hanya boleh huruf dan spasi
  if (!/^[a-zA-Z\s]+$/.test(trimmed)) return 'Fullname can only contain letters and spaces';
...
export function identifier(value: string): string | null {
  ...
  if (!value.trim()) return 'User identifier cannot be empty';
